...
> + Clone {
    recursive(|expr| {
        ...
        let rnd_lit = just(Token::Rnd).ignore_then(
            expr.clone().then_ignore(just(Token::Comma)).then(expr.clone())
                .delimited_by(just(Token::LParen), just(Token::RParen))
        ).map_with(|(low, high), info| {
            (Expr::Rnd(Box::new(low), Box::new(high)), info.span())
        });
